...
上記のコードスニペットの例 (1 件目) 中の `'/\buy'` という文字列は、バックスペース制御文字 `\b` を含む ctx7 の実出力どおりの転記である。壊れた文字列に見えるが誤字ではない。直さないこと。info スニペットの例 (4 件目) は、この後に続くエントリと切り分ける区切り線の直前までを掲載しており、エントリ内で省略した部分は無い。
