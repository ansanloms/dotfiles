...
- この手順が縛るのは worktree のライフサイクル、すなわちセットアップ (用意 + description 設定)・作業の節目での description 更新 (下記「更新」)・マージ後の片付け (下記「片付け」) の 3 段階。その間に行うコミット・push・PR 自体は対象外で、通常の Git 運用 (コミット規約等) に従う。
...
description は書いた時点の記録ではなく、worktree の現在地を示すダッシュボードとして扱う。一覧ツールのプレビューは description をそのまま表示するため、更新を怠ると一覧が実態と乖離する。
...
- 更新手順:
  ...
  3. 「書式」の設定方法と同じ手段 (改行を含む値を `git config` へ渡す) で再設定する。全文を渡すため、変更しない部分も含めて完全な内容を設定する。
